...
class StartDockerImagePerformer extends Stage {
    ...
    void executeImpl(StageContext ctx) {
        try {
            ...
            ctx.env.execute("DOCKER_HOST=172.23.104.43:2376 docker kill performer")
        }
        catch(RuntimeException err) {
        }

        //TODO change "performer" to an image name or something, the driver isnt playing well with having the image name as a name atm
        ctx.env.execute("DOCKER_HOST=172.23.104.43:2376 docker run --rm --network perf -d -p $port:8060 --name performer $imageName version=$version")
    }
